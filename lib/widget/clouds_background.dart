...
class Clouds extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var screenSize = MediaQuery.of(context).size; /// Because the clouds move all
...
class CloudPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    /// placement des rectangles en dessous des nuages
    var rectTop = 110.0;
    var rectBottom = rectTop + 40.0;


    /// placement des cercles, on se repère à l'axe horizontal de l'écran pour les placer
    var figureLeftEdge = size.width / 4;
    var figureRightEdge = size.width - 90.0;
    var figureCenter = size.width / 2;

    /// Rect.fromPponts creates a rectangle from the top-left and bottom-right corner offsets
    var cloudBaseRect = Rect.fromPoints(
      Offset(figureLeftEdge, rectTop),
      Offset(figureRightEdge, rectBottom),
    );
    /// RRect is a rectangle with rounded corners
    var cloudBase = RRect.fromRectAndRadius(
      cloudBaseRect,
      Radius.circular(10.0),
    );
    /// Tell canvas to actually execute the painting
    ...
    canvas.drawRRect(cloudBase, cloudPaint); /// Peint le RRect
